#ifndef UE4SS_SDK_BP_BuildObject_Ancient_WallGate_HPP
#define UE4SS_SDK_BP_BuildObject_Ancient_WallGate_HPP

class ABP_BuildObject_Ancient_WallGate_C : public APalBuildObjectBasicBase
{
    class USphereComponent* ConnectPointCollision;                                    // 0x0780 (size: 0x8)
    class UBP_BuildObjectSimulateArrowComponent_C* BP_BuildObjectSimulateArrowComponent; // 0x0788 (size: 0x8)
    class UBoxComponent* FrameCollision_01;                                           // 0x0790 (size: 0x8)
    class UBoxComponent* FrameCollision_02;                                           // 0x0798 (size: 0x8)
    class UBP_InteractableBox_C* BP_InteractableBox;                                  // 0x07A0 (size: 0x8)
    class UBoxComponent* BuildWorkableBounds;                                         // 0x07A8 (size: 0x8)
    class UStaticMeshComponent* SM_Wall_Wood;                                         // 0x07B0 (size: 0x8)
    class USceneComponent* Root;                                                      // 0x07B8 (size: 0x8)
    class UBoxComponent* CheckOverlapCollision;                                       // 0x07C0 (size: 0x8)

    void GetStaticMeshComponents(TArray<class UStaticMeshComponent*>& OutComponents);
    void GetStaticMeshInfos(TArray<FPalStaticMeshImposterStaticMeshInfo>& OutStaticMeshInfo);
}; // Size: 0x7C8

#endif
